...
from relax.network.fm import FMNet, Diffv2Params
    ...
    def __init__(
        ...
        *,
        ...
        def stateless_update(
            ...
            next_eval_key, flow_noise_key, r_key, mask_key, t_key, flow_noise_key2, t_key2 = jax.random.split(
                key, 7)
            ...
            def policy_loss_fn(policy_params) -> jax.Array:
                ...
                loss_d = self.agent.flow.weighted_pd_loss(flow_noise_key, q_weights, denoiser, r_final, t_final,
                                                            jax.lax.stop_gradient(next_action))
                
                t_b = jax.random.uniform(t_key2, shape=(next_obs.shape[0],), minval=0.0, maxval=1.0)
                loss_b = self.agent.flow.weighted_pb_loss(flow_noise_key2, q_weights, denoiser, t_b,
                                                            jax.lax.stop_gradient(next_action))
                
                loss = 0.2 * loss_b + loss_d
