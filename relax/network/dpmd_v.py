...
from relax.network.blocks import Activation, DACERPolicyNet_V, QNet_V, EncoderNet
...
class DPMDNet_V:
    ...
    #ddim sampler with one sample step
    # def get_one_step_action(self, key: jax.Array, policy_params: hk.Params, obs: jax.Array) -> jax.Array:
    ...
    #     def sample(key: jax.Array) -> Union[jax.Array, jax.Array]:
    #         act = self.diffusion.p_sample_ddim(key, model_fn, (*obs.shape[:-1], self.act_dim))
    #         return act.clip(-1, 1)
...
def create_dpmd_net_visual(
    ...
    ) -> Tuple[DPMDNet_V, Diffv2Params]:
    q = hk.without_apply_rng(hk.transform(lambda obs, act: QNet_V(hidden_sizes, activation)(obs, act)))
    policy = hk.without_apply_rng(hk.transform(lambda obs, act, t: DACERPolicyNet_V(diffusion_hidden_sizes, activation)(obs, act, t)))
    ...
    net = DPMDNet_V(q=q.apply, policy=policy.apply, encoder=encoder.apply, num_timesteps=num_timesteps, act_dim=act_dim, 
