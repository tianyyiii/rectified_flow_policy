...
from relax.utils.flow import ShortCutModel, LagrangianModel, MeanFlowModel
...
class FMNet:
    ...
    def flow(self):
        if self.flow_map_type == "shortcut":
            return ShortCutModel(self.num_timesteps)
        elif self.flow_map_type == "lagrangian":
            return LagrangianModel(self.num_timesteps)
        elif self.flow_map_type == "meanflow":
            return MeanFlowModel(self.num_timesteps)
...
def create_fm_net(
    ...
    flow_map_type: str = "shortcut",
    ) -> Tuple[FMNet, Diffv2Params]:
    ...
    net = FMNet(q=q.apply, policy=policy.apply, num_timesteps=num_timesteps, num_timesteps_test=num_timesteps_test, act_dim=act_dim, 
                    target_entropy=-act_dim*target_entropy_scale, num_particles=num_particles, noise_scale=noise_scale,
                    noise_schedule='linear', flow_map_type=flow_map_type)
    return net, params
