...
class RF2Net:
    ...
    def get_action_coupled(self, key: jax.Array, policy_params: hk.Params, obs: jax.Array) -> jax.Array:
        ...
        def sample(key: jax.Array) -> Union[jax.Array, jax.Array]:
            acts = self.flow.p_sample_traj(key, model_fn, (*obs.shape[:-1], self.act_dim))
            act = acts[-1, ...]
            q1 = self.q(q1_params, obs, act)
            q2 = self.q(q2_params, obs, act)
            q = jnp.minimum(q1, q2)
            return act.clip(-1, 1), acts[0, ...], q

        key, noise_key = jax.random.split(key)
        if self.num_particles == 1:
            act, act0, _ = sample(key)
        else:
            keys = jax.random.split(key, self.num_particles)
            acts, act0s, qs = jax.vmap(sample)(keys)
            q_best_ind = jnp.argmax(qs, axis=0, keepdims=True)
            act = jnp.take_along_axis(acts, q_best_ind[..., None], axis=0).squeeze(axis=0)
            act0 = jnp.take_along_axis(act0s, q_best_ind[..., None], axis=0).squeeze(axis=0)
...
def create_rf2_net(
    ...
    net = RF2Net(q=q.apply, policy=policy.apply, num_timesteps=num_timesteps, num_timesteps_test=num_timesteps_test, act_dim=act_dim, 
